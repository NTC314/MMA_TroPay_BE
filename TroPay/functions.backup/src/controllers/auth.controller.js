...
const register = async (req, res) => {
  try {
    const { phone, password, full_name, email, role = 'tenant' } = req.body;

    // Validate role
    if (role && !['owner', 'tenant', 'admin'].includes(role)) {
      return res.status(400).json({
        success: false,
        message: 'Invalid role. Must be owner, tenant, or admin'
      });
    }
    ...
    logger.info(`New user registered: ${user._id} with role: ${role}`);
...
const login = async (req, res) => {
  try {
    const { phone_or_email, password, role } = req.body;
    ...
    if (role && user.role !== role) {
      return res.status(401).json({
        success: false,
        message: `Invalid role. Expected ${role} but user is ${user.role}`
      });
    }
    ...
    logger.info(`User logged in: ${user._id} with role: ${user.role}`);
